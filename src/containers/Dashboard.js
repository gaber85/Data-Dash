...
class Dashboard extends Component {
  ...
  async getSurfaceSeaWaterSpeed () {
    ...
    Object.keys(jsonData).sort().forEach(key => {
      if (key && jsonData[key].surface_sea_water_speed) {
        surfaceSeaWaterSpeed.push({key,value:jsonData[key].surface_sea_water_speed})
      }
    });
    await this.setState({
      data: {
        labels: surfaceSeaWaterSpeed.map(entry => moment(entry.key).format('MMM D, h:mm:ss a')),
        datasets: [
          {
            label: 'Speed m/s',
  ...
  render() {
    ...
    return (
      <div>
        <LineGraph graphData={data} text="Surface Seawater Speed (Nov 7 - 14 2018)"/>
      </div>
      );
  } 
}
